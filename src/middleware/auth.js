...
//
// Rules:
//   • If the user does NOT have 2FA enabled → pass through (they can't have a
//     mfaVerifiedAt stamp but also have no second factor to complete).
//   • If the user HAS 2FA enabled → their token MUST contain mfaVerifiedAt and
//     that timestamp must be within MAX_MFA_AGE_HOURS hours.
//   • If the check fails → 403 with a structured error so the frontend can
//     redirect to the OTP page rather than showing a generic error.
//
  ...
  return async (req, res, next) => {
    ...
    if (!claims.mfaVerifiedAt) {
      return res.status(403).json({
        success: false,
        code: 'MFA_REQUIRED',
        message: 'This page requires two-factor authentication. Please verify your identity.',
      });
    }

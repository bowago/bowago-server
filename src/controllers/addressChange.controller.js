...
async function requestAddressChange(req, res) {
  ...
  if (blocked.includes(shipment.status)) {
    const atPickup = shipment.status === "PICKED_UP";
    const inTransit = ["IN_TRANSIT", "OUT_FOR_DELIVERY"].includes(
      shipment.status,
    );
    const message = atPickup
      ? "Cannot change address — your package has already been collected by the driver. Contact support."
      : inTransit
        ? "Cannot change address while shipment is in transit. Contact support or file a claim after delivery."
        : `Cannot request address change for a shipment with status: ${shipment.status}`;
  ...
  if (existing) {
    throw new ApiError(
      409,
      "There is already a pending address change request for this shipment",
    );
  }
...
async function reviewAddressChange(req, res) {
  ...
  if (action === "APPROVE") {
    ...
    await prisma.shipment.update({
      ...
      data: {
        status:
          changeReq.shipment.status === "PENDING_ADMIN_REVIEW"
            ? "CONFIRMED"
            : changeReq.shipment.status,
      },
    });
  }

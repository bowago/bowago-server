...
async function createPriceAdjustment(req, res) {
  ...
  if (
    proofImageUrls &&
    Array.isArray(proofImageUrls) &&
    proofImageUrls.length > 0
  ) {
    if (proofImageUrls.length > 5)
      throw new ApiError(400, "Maximum 5 proof images allowed");
    imageUrls = proofImageUrls;
  } else if (proofImageUrl) {
    imageUrls = [proofImageUrl];
  }

  const windowHours = await getNumberSetting(
    "price_adjustment.response_window_hours",
  );
  ...
  const adjustmentNotification = await prisma.notification.create({
    data: {
  ...
  notify(shipment.customerId, adjustmentNotification);

  return created(
    res,
    { adjustment },
    "Price adjustment created. Customer has been notified.",
  );
}
...
async function acknowledgePriceAdjustment(req, res) {
  ...
  if (adjustment.shipment.customerId !== req.user.id)
    throw new ApiError(403, "Access denied");
  if (isResolved(adjustment))
    throw new ApiError(
      400,
      `This adjustment is already ${adjustment.status.toLowerCase()}`,
    );
...
async function downgradePriceAdjustment(req, res) {
  ...
  const downgradeEnabled = await getBoolSetting(
    "price_adjustment.downgrade_enabled",
  );
  if (!downgradeEnabled)
    throw new ApiError(400, "Downgrade option is currently disabled");

  if (!TIER_ORDER.includes(newServiceType)) {
    throw new ApiError(
      400,
      `newServiceType must be one of: ${TIER_ORDER.join(", ")}`,
    );
  }
  ...
  if (newTierIdx >= currentTierIdx) {
    throw new ApiError(
      400,
      "Downgrade must select a lower service tier than the current one",
    );
  }
  ...
  if (delta > 0) {
    paymentResult = await initializePayment({
      ...
      email: req.user.email,
      metadata: {
        type: "PRICE_ADJUSTMENT_DOWNGRADE",
        adjustmentId: adjustment.id,
  ...
  const downgradeNotification = await prisma.notification.create({
    data: {
      userId: shipment.customerId,
      type: "PRICE_ADJUSTMENT",
  ...
  notify(shipment.customerId, downgradeNotification);

  return success(
    res,
    { newPrice, delta, paymentResult, refundResult },
    "Downgrade processed",
  );
}
...
async function cancelPriceAdjustment(req, res) {
  ...
  const refundPercent = await getNumberSetting(
    "price_adjustment.cancel_refund_percent",
  );
...
async function cancelAndRefund({
  ...
  await prisma.notification
    ...
    .then((n) => notify(shipment.customerId, n))
    .catch(() => {});
...
async function contactSupportForAdjustment(req, res) {
  ...
  req.body = {
    ...
    body:
      message ||
      `Customer requested support regarding a price adjustment of ₦${adjustment.difference.toLocaleString()} on shipment ${adjustment.shipment.trackingNumber}.`,
    priority: "HIGH",
  };
...
async function extendAdjustmentDeadline(req, res) {
  ...
  const base =
    adjustment.responseDeadline && adjustment.responseDeadline > new Date()

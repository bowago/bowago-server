...
async function getQuote(req, res) {
  const { fromCity, toCity, weightKg, tons, cartons, boxDimensionId } = req.body;

  const quote = await calculateShippingCost({ fromCity, toCity, weightKg, tons, cartons, boxDimensionId });
...
async function deleteDimension(req, res) {
  const { id } = req.params;
  await prisma.boxDimension.delete({ where: { id } });
...
async function listPriceBands(req, res) {
  ...
  const bands = await prisma.priceBand.findMany({
    where: zone ? { zone: parseInt(zone) } : {},
    orderBy: [{ zone: 'asc' }, { minKg: 'asc' }],
  });
  return success(res, { bands });
}

async function createPriceBand(req, res) {
  const { zone, minKg, maxKg, minTons, maxTons, minCartons, maxCartons, pricePerKg, basePrice } = req.body;

  const band = await prisma.priceBand.create({
    data: { zone, minKg, maxKg, minTons, maxTons, minCartons, maxCartons, pricePerKg, basePrice },
  });
...
async function importPricingSheet(req, res) {
  ...
  if (workbook.SheetNames.includes('Dimensions')) {
    ...
    // Find the CSV-style rows (starting with category_id header)
    let csvStartIdx = -1;
    for (let i = 0; i < sheet.length; i++) {
      if (sheet[i][0] && String(sheet[i][0]).startsWith('category_id')) {
        csvStartIdx = i + 1;
        break;
      }
    }
        ...
        try {
          ...
          await prisma.boxDimension.upsert({
            ...
            update: {
              ...
              weightKgLimit: parseFloat(weightKgLimit),
            },
            create: {
          ...
          results.errors.push(`Dimension row ${i}: ${e.message}`);
        }
      }
    }
  }
  ...
  if (workbook.SheetNames.includes('Zone Matrix')) {
    ...
    const headers = sheet[0]; // ['SNo', 'City', 'Aba', 'Abakaliki', ...]
    const cityColumns = headers.slice(2); // city names as columns

    // Ensure all cities exist
    const cityMap = {};
    const allCityNames = new Set([...cityColumns]);
    for (let i = 1; i < sheet.length; i++) {
      if (sheet[i][1]) allCityNames.add(sheet[i][1]);
    }
    ...
    for (const cityName of allCityNames) {
      ...
      const city = await prisma.city.upsert({
        ...
        update: {},
        create: { name: cityName.trim(), region: 'Unknown', state: 'Unknown' },
      });
    ...
    for (let i = 1; i < sheet.length; i++) {
      ...
      for (let j = 0; j < cityColumns.length; j++) {
        ...
        try {
          ...
          results.errors.push(`Zone ${fromCityName}->${toCityName}: ${e.message}`);
        }
      }
    }
  }
    ...
    for (let i = 1; i < sheet.length; i++) {
      if (sheet[i][0]) {
        const city = await prisma.city.findFirst({
          where: { name: { equals: sheet[i][0].trim(), mode: 'insensitive' } },
        });
    ...
    for (let i = 1; i < sheet.length; i++) {
      ...
      for (let j = 0; j < toCityNames.length; j++) {
        ...
        try {
          ...
          results.errors.push(`KM ${fromCityName}->${toCityName}: ${e.message}`);
        }
      }
    }
  }

  // ── Sheet: Zone Matrix by Region ───────────────────────────────────────────
  if (workbook.SheetNames.includes('Zone Matrix by Region')) {
    const sheet = XLSX.utils.sheet_to_json(workbook.Sheets['Zone Matrix by Region'], { header: 1 });
    let currentRegion = '';

    for (let i = 1; i < sheet.length; i++) {
      const row = sheet[i];
      if (row[0] && typeof row[0] === 'string') currentRegion = row[0].trim();
      const cityName = row[1];
      if (!cityName) continue;

      // Update city region info

...
function twoCol(
  ...
  value,
  y,
  labelColor = BRAND.gray,
  valueColor = BRAND.dark,
) {
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fontSize(9)
    .text(value, 300, y, { align: "right", width: 245 });
}
...
async function generateInvoicePDF(data) {
  const {
    ...
    appliedDiscount, // { label, discountAmount } | null — contract rate / promo, if applied
  } = data;
  ...
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      doc
        ...
        .font("Helvetica-Bold")
        .fontSize(9)
        .text(formatNaira(row.amount), 450, rowY, {
          align: "right",
          width: 90,
        });
    ...
    // ─── Applied discount note ──────────────────────────────────────────────
    // shipment.quotedPrice (used as "Subtotal" above) is the FINAL,
    // already-discounted price — the discount was applied before this PDF
    // ever ran, not something to subtract again here. So this is shown as
    // an informational note rather than another line item in the totals
    // math, to avoid implying the customer is owed a further reduction.
    if (appliedDiscount) {
      ...
      doc
        ...
        .fontSize(9)
        .text(
          `✓  ${appliedDiscount.label} applied — you saved ${formatNaira(appliedDiscount.discountAmount || 0)} (already reflected in the price above)`,
          58,
          doc.y + 7,
        );
      doc.y += 34;
    ...
    if (isPaid && payment?.paidAt) {
      ...
      doc
        ...
        .fontSize(10)
        .text(
          `✓  Payment received on ${formatDate(payment.paidAt)} via ${payment.channel || "card"}`,
          58,
          doc.y + 7,
        );
      doc.y += 34;
...
async function generateShippingLabelPDF(shipment) {
  return new Promise((resolve, reject) => {
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(9)
      .text(shipment.serviceType || "STANDARD", 202, 22, {
        align: "center",
        width: 69,
      });
    ...
    doc
      ...
      .font("Helvetica")
      .fontSize(7)
      .text("TRACKING NUMBER", 15, 48);
    doc
      .fillColor(BRAND.dark)
      .font("Helvetica-Bold")
      .fontSize(16)
      .text(shipment.trackingNumber, 15, 58);
    ...
    doc
      ...
      .text(`${shipment.senderCity}, ${shipment.senderState}`, 15, 120, {
        width: 200,
      });
    ...
    doc
      ...
      .text(`${shipment.recipientCity}, ${shipment.recipientState}`, 15, 183, {
        width: 258,
      });
    doc
      .fillColor(BRAND.gray)
  ...
  return new Promise((resolve, reject) => {
    ...
    if (shipment.cutoffWarning) {
      ...
      doc
        ...
        .fontSize(9)
        .text(
          "⚠  Booked after 2:00 PM — earliest pickup is the next business day.",
          58,
          doc.y + 9,
        );
      doc.y += 36;
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(11)
      .text("Shipment Details", 50, detailsY);
    doc.y += 18;
    drawRule(doc, doc.y);
    doc.y += 12;
    ...
    const details = [
      ...
      [
        "To",
        `${shipment.recipientName} — ${shipment.recipientCity}, ${shipment.recipientState}`,
      ],
    ...
    doc
      ...
      .text(
        "3.  Have your package ready at the pickup address on the scheduled date.",
        60,
        doc.y + 52,
      );
